...
path = data_dir / 'processed' / 'luce_nat_es_hw_hist'  #  UV  luce_ult_es2  luce_nat_es_hw_hist
...
# train_classification_model(ml_df, non_features, verbose=4)

# combined_regression, scaler_X, scaler_y = 
combined_regression, scaler_X, scaler_y = train_regression_model(ml_df, non_features, verbose=4)
...
path = data_dir / 'processed' / 'luce_uv_inc'  # luce_nat_inc  luce_uv_inc
df_file = path /  ml_df_name
assert df_file.is_file()
ml_df_FULL = pd.read_pickle(str(df_file))
...
# X = combine_class(X, 'proba',  combined_classifier, None) 
# X = pd.get_dummies(X)

y_pred = scaler_y.inverse_transform(combined_regression.predict(X).reshape(-1, 1)).ravel()
y_pred = pd.Series(y_pred, index=X.index, name='comb_pred')

#  X_classes = X[X.columns[-6:]]


y_pred = pd.DataFrame({'well_plate_name': ml_df_inc.well_plate_name,
                       'well_name': ml_df_inc.well_name,
                       'wp_version': ml_df_inc.wp_image_version,
                       # 'class': X_classes[X_classes.idxmax(axis=1)].columns,
                       'value': y_pred}, index= X.index)
