...
output_images = reportings_dir / 'figures' / 'luce_nat' # luce_nat luce_nat_es_hw luce_nat_es_hw_hist
...
plt.close('all')
cv2.imwrite(str(output_images/'imgR.jpg'), imgR)
...
cv2.imwrite(str(output_images/'mask_query_circ.jpg'), mask_query_circ)
imgR_single_channel = single_channel_equalize(imgR_structure, c_map = 'gray',  eq_method = 'clahe')
cv2.imwrite(str(output_images/'imgR_single_channel.jpg'), imgR_single_channel)

keypoints_reference, descriptions_reference, model = calculate_keypoints(imgR_single_channel, graphics=True, save_name=str(output_images/'img_sift.jpg'))
...
data_dict_df = {}
path_train = data_dir / 'raw' / 'ESWAexp_v2' / 'luce_nat' #luce_nat_es_hw luce_nat_es_sam
output_folder = data_dir / 'intermediate' / 'ESWAexp_v2_crp' / 'luce_nat' # luce_nat luce_nat_es_hw luce_nat_es_hw_hist
...
for folder in path_train.iterdir():
    ...
    for jpg in folder.rglob('*.jpg'): #TODO add jpeg
        ...
        target_path = out_fold / (image_name + '_horota.jpg')  
        ...
        target_path = out_fold / (image_name + '_singcha.jpg') 
        cv2.imwrite(str(target_path), img1_single_channel)
        target_path = out_fold / (image_name + '_img_sift.jpg') 
        print(f'Out Folder {target_path}', end='\n')
        keypoints1_query, descriptions1_query, model= calculate_keypoints(img1_single_channel, graphics=True, save_name=str(target_path))
        ...
        while (err>err_tresh and lowe_tre<0.75):
            ...
            info_dict = matching_info(imgR_single_channel, keypoints_reference, img1, keypoints1_query,  matrix, M, matrix_mask, matches, show = True, save = True, 
