...
if _local_dir:
    ...
    data_origianl_dir = Path(__file__).resolve().parents[4].joinpath('MultiWell','data')

else:
    data_dir = Path(__file__).resolve().parents[4].joinpath('Projects','MultiWell', 'data')
    raw_data_dir = Path(__file__).resolve().parents[4].joinpath('Projects', 'MultiWell', 'data', 'raw')
    reportings_dir = Path(__file__).resolve().parents[4].joinpath('Projects', 'MultiWell', 'reports')
    log_dir = Path(__file__).resolve().parents[4].joinpath('Projects', 'MultiWell', 'data', 'log_files')
