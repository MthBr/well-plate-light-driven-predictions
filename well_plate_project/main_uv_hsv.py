...
from well_plate_project.preprocessing import match_target_feat, extract_img_tags, extract_featuresAB, resize_image
...
image_file_structure = path_query / 'UV_inc_hole.jpg' #1ln_b 2uv_b  empty_UV
assert image_file_structure.is_file()
imgR_structure = cv2.imread(str(image_file_structure)) 
plt.imshow(imgR_structure)

#%% IMPORT FILES
image_file = path_query / 'UV_inc_whole.jpg'  #1ln  2uv  empty_UV UV_big
...
plt.close('all')
mask_query_circ, reference_wells = circle_extract(single_channel_equalize(imgR, c_map = 'HSV', channel = [0,1,0]),
                                                  dp = 4.32, param1=150, param2=None, plot=True) #2.93 4.351
imgR_single_channel = single_channel_equalize(imgR_structure, eq_method = 'clahe', c_map = 'LAB',  channel = [1,0,0])
...
data_dict_df = {}
path_train = data_dir / 'raw' / 'exp_v2' / 'luce_uv_inc'
output_folder = data_dir / 'raw' / 'exp_v2_crp' / 'luce_uv_inc' # EXPERIMENTS_Crp  foto_tel1_crp
...
for folder in path_train.iterdir():
    ...
    for jpg in folder.rglob('*.jpg'):
        ...
        img1_single_channel = single_channel_equalize(img1, eq_method = 'clahe', c_map = 'LAB',  channel = [1,0,0])
...
output_folder = data_dir / 'processed' / 'luce_uv_inc'
